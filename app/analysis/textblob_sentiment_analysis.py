...
df["sentiment"], df["sentiment_score"] = zip(
    *df["review"].apply(get_textblob_sentiment)
)
...
print(f"✅ TextBlob sentiment analysis saved to {args.file_output}")
...
ax.set_title("Sentiment Distribution by Topic (TextBlob)")
ax.set_xticks([i + bar_width for i in index])
ax.set_xticklabels(sentiment_dist["label"], rotation=30, ha="right")
ax.legend()

plt.tight_layout()
plt.savefig("app/data/topics/sentiment_distribution_textblob.png")
print("✅ Sentiment distribution plot saved to 'sentiment_distribution_textblob.png'")
